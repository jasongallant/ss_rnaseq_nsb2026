...
::: {.callout-note title="A contamination axis we can check: ribosomal content"}
...
```{r percent-ribo, message=FALSE, warning=FALSE}
merged_seurat[["percent_ribo"]] <- PercentageFeatureSet(
  merged_seurat, pattern = "^rp[sl]"
)

summary(merged_seurat$percent_ribo)
```

Most nuclei are near zero with a small upper tail — as expected for a nuclear prep. Split by sample, the distributions look unremarkable; the contamination signal isn't a whole-sample property, which is precisely why we defer the closer look until we can examine it cluster by cluster.
...
```{r percent-ribo-featureplot, fig.width=7, fig.height=6, message=FALSE, warning=FALSE, fig.alt="FeaturePlot of per-nucleus ribosomal-protein percentage on the Harmony UMAP, with one peripheral cluster highlighted in warm color.", fig.cap="Ribosomal-protein content projected onto the Harmony UMAP. Nearly all nuclei are near zero (grey); a single peripheral cluster stands out in warm color, marking it as enriched for cytoplasmic ribosomal-protein transcript."}
...
```{r percent-ribo-by-cluster, message=FALSE, warning=FALSE}
merged_seurat@meta.data |>
  dplyr::group_by(seurat_clusters) |>
  dplyr::summarise(median_ribo = median(percent_ribo), n = dplyr::n()) |>
  dplyr::arrange(dplyr::desc(median_ribo))
```

One cluster sits at roughly **9% median ribosomal content — an order of magnitude above the next cluster, and ~30–50× the bulk of the atlas (~0.2%)**. That is not a gradient; it's a categorical outlier.

::: {.callout-note title="Why this cluster is a contamination artifact, not a cell type"}
...
1. CellRanger's cell calls are trustworthy for the EO library; we did not apply manual UMI/gene/mitochondrial filtering. We *surfaced* a ribosomal-content signal (a cytoplasmic-contamination axis specific to snRNA-seq) — one peripheral cluster stands out — but used it to interpret, not to filter.
